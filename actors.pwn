...
{
	foreach(new  x : Actor) DestroyActor(x);
	db_close(db_actors);
	return 1;
}
...
CMD:applyplayeranimation(playerid, args[]) {
	...
	format(str, sizeof(str), ">> Pomyœlnie nada³eœ sobie animacjê [%s] z biblioteki [%s]", animname, animlib);
...
CMD:actorhelp(playerid) {
	...
	{F9CC00}/applyactoranimation\t/aanim\tNadaje animacjê aktywnemu aktorowi.\n\
	{F9CC00}/clearactoranimation\t/clearaa\tUsuwa animacjê aktywnemu aktorowi.\n\
	{F9CC00}/moveactor\t-\tZmienia pozycjê aktywnego aktora.";
	strcat(string, "\n{F9CC00}/actorrotation\t/actorrot\tZmienia rotacjê aktywnego aktora.");
...
CMD:applyactoranimation(playerid, args[]) 
{
	...
	if(actorid == INVALID_ACTOR_ID) return SendClientMessage(playerid, COLOR_RED, ">> Nie wykryto ¿adnego aktywnego aktora.");
	if(sscanf(args, "s[64]s[64]ii", animlib, animname, looped, time)) return SendClientMessage(playerid, COLOR_BLUE, "U¯YCIE: /applyactoranimation [biblioteka animacji] [nazwa animacji] [zapêtlona - 0/1] [czas trwania w ms]");
